...
Status: Workstream 04 PIT-001/PIT-002 pass through governed pitch evidence; PIT-003...008 and full Workstream 12, external, hardware, host, listening, and release validation remain open.
...
| F0 — reproducible baseline | [12 Reference/Test harness](12-hardware-reference-regression-system.md) | F0 skeleton complete and extended by Workstream 04: five deterministic fixtures, four versioned analyzers, 21 governed inputs, a globally draft acceptance policy, exact 127-requirement map, 110-gate canonical report, and release enforcement are committed and verified. TST-001–009 remain individually `in-progress`. | Workstream 02 test/offline targets and Workstream 03 automated contracts are stable | Continue full Workstream 12 completion through F4 while retaining every owner/pre-release gap. |
| F1 — processor seam freeze | [04 Pitch, Tuning, Glide, and Modulation](04-pitch-tuning-glide-modulation.md) | In progress: [PIT-001/PIT-002 pass](evidence/workstream-04/pit-001-pit-002-pitch-foundation.md) through typed semitone-domain production math, governed processor renders, nine bound gates, authoritative replay, and corrected strongest-peak analysis. PIT-008 still owns static scheduling/caching. PIT-003...008 remain open; the report is 16 pass / 92 not-run / 19 awaiting / 0 fail and non-release-ready. | Accepted F0 harness plus frozen Workstream 03 contracts | Plan PIT-003 then PIT-004 in roadmap order; retain PIT-005...008 and all parallel BLD/PAR/TST pre-release obligations. |
...
- `MoogMiniAudioProcessor::processBlock` cross-routes filter/loudness contour controls, processes all MIDI before rendering the block, stops oscillators on note-off before release, uses a single `currentNoteNumber`, performs repeated per-sample filter and still-open PIT-008 pitch-control/composition work, applies filter modulation in linear hertz, and reads a `feedbackKnob` that does not create an audible feedback path. PIT-001/PIT-002 now route musical pitch through the typed semitone authority without claiming the remaining lifecycle, modulation, glide, static scheduling, or caching work.
- Musical Oscillator 2/3 pitch now maps the `Frequency` enum index (0–16) to −8…+8 through `PitchDomain`; the deliberately preserved LO compatibility path remains pending calibration. `Oscillator::processNextSample` still directly emits discontinuous saw/square/pulse waveforms, allocates a `juce::String` per sample, and uses complementary 70/30 pulse shapes.
