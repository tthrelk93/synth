...
Use only `not-started`, `in-progress`, `blocked`, `fail`, or `pass`. A requirement may be `pass` only when its acceptance criterion and verification evidence are linked. Agent 04 owns these entries under the current handoff.
